# -*- coding: utf-8 -*-
"""
Created on Wed May 29 11:01:06 2019
...
print("accuracy by score :",classifier.score(features_test,labels_test)*100)

# What percentage of total women actually had an affair?*****************************************
women_affair = (( cm[1][1] + cm[1][0] )/(cm[0][0] + cm[1][1] + cm[0][1] + cm[1][0]) ) * 100
print("Women had an affair :",women_affair)
#or
dataset['affair'].value_counts(normalize = True)[1]
...
import numpy as np
input_data = [4,25,3,1,4,16,4,2]
input_data = np.array(input_data)
input_data = input_data.reshape(1,8) #convert to 1x8 ndarray

input_data_onehot = onehotencoder.transform(input_data).toarray() #convert with onehot
input_data_feature = sc.transform(input_data_onehot)#perform feature


print("Score :",classifier.score(features_train, labels_train)) #score gives result from actual train data

print(classifier.predict(input_data_feature)) #find prediction after training
print(classifier.predict_proba(input_data_feature)) #find probability after training
...
print("Accuracy is :- ",((cm[0][0]+cm[1][1])/(cm[0][0]+cm[0][1]+cm[1][0]+cm[1][1]))*100,"%")
...
#**********************************************************************************************
# by decision tree and random forest
import pandas as pd
import numpy as np
