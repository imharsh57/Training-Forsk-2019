Q1. This is the guides code challenge where you have to first scrapp images from the internet and 
use them for training the CNN model.
...
# df = dataset.nlargest(5,'%') -->top 5percentage in seperate column
#df = dataset.groupby(['State','Constituency']).first() -->print the df

''' 1. Fetch the top parties of each state within each constituency with their vote %. '''

df = dataset.groupby(['State','Constituency'])
df = df['%'].max()

for i in df:
    print(dataset[dataset['%']==i][['State','Constituency','Party','%']])
...
#df['Index'] = df['Index']+","+df['Party']


dataset1 = dataset 

df1 = dataset[dataset['State']=="Rajasthan"]
#grouping and find max vote %
df1 = df1.groupby(['State','Constituency'])
df =df1['%'].max()
#finding the party list of rajasthan with highest vote %
party = df1.first()
party = list(party['Party'])

#df_party=pd.DataFrame(data=party, columns=['Party']) 


df = df.append(df_party)


constituency = list(df.index)
percentage = list(df)
...
#******************************************************************************************
#1. Fetch the top parties of each state within each constituency with their vote %.
dataset1 = dataset.sort_values('%').drop_duplicates(['State','Constituency'],keep='last')
party_name = dataset1.iloc[:,[2,6,7,8]]

#******************************************************************************************
# 2. Visualize the top parties vote % in each constituency for Rajasthan.

dataset2 = dataset[dataset['State']=="Rajasthan"].sort_values('%').drop_duplicates(['State','Constituency'],keep='last')
party_name2 = dataset2.iloc[:,[2,6,8]]
party_name2['Party'] = party_name2['Party'] + "," + party_name2['Constituency']


import matplotlib.pyplot as plt
ax = party_name2.plot(x='Party', y='%',kind='bar', title ="VISULATION", figsize=(9, 4), legend=True, fontsize=10)
...
#****************************************************************************************
# 3. Visualize the total seats gained by each party in each states.
seats1 = dataset.groupby('State')['Party'].value_counts()

#****************************************************************************************
# 4. Visualize the total seats won by the parties in the whole country
seats2 = dataset1.groupby('State')['Party'].value_counts()